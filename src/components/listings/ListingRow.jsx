...
export default function ListingRow({ listing, isActive, onMouseEnter, onMouseLeave }) {
  ...
  return (
    <Link
      ...
      className={`flex bg-white rounded-xl border overflow-hidden shadow-sm hover:shadow-md transition-all duration-150 group ${isActive ? "border-[#1A73E8] shadow-md" : "border-[#E0E0E0]"}`}
    >
      {/* ── Image ───────────────────────────────────────── */}
      <div className="relative w-48 shrink-0 overflow-hidden">
        ...
        <button
          ...
          className={`absolute top-2 right-2 p-1.5 rounded-full shadow transition-colors ${saved ? "bg-red-500 text-white" : "bg-white/90 text-[#5F6368] hover:text-red-500"}`}
        >
      ...
      <div className="flex flex-col flex-1 p-4 min-w-0">
        ...
        {/* Phone */}
        <p className="flex items-center gap-1.5 text-xs text-[#1A73E8] font-medium mb-3">
          <PhoneIcon />{phone}
        </p>
